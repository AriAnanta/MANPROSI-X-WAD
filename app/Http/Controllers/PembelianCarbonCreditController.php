...
class PembelianCarbonCreditController extends Controller
{
    ...
    public function index()
    {
        ...
        $carbon_credit = PembelianCarbonCredit::where('kode_admin', $kodeAdmin)
    ...
    {
        // Ambil semua data pembelian carbon credit
        $carbonCredits = PembelianCarbonCredit::with(['admin', 'kompensasiEmisi'])
            ->orderBy('tanggal_pembelian_carbon_credit', 'desc')
            ->get()
            ->map(function ($credit) {
                return [
                    'kode_pembelian' => $credit->kode_pembelian_carbon_credit,
                    'kategori' => $credit->kompensasiEmisi->kategori ?? '-',
                    'sub_kategori' => $credit->kompensasiEmisi->sub_kategori ?? '-',
                    'jumlah_kompensasi' => $credit->jumlah_kompensasi,
                    'tanggal_pembelian' => $credit->tanggal_pembelian_carbon_credit,
                    'status_kompensasi' => $credit->kompensasiEmisi->status ?? 'pending',
